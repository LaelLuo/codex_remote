const STORE_KEY = "__codex_remote_i18n_store__";
const STORAGE_KEY = "codex_remote_locale";

export type Locale = "en" | "zh-CN";

const SUPPORTED_LOCALES: Locale[] = ["en", "zh-CN"];

type MessageDict = Record<string, string>;
export type TranslationParams = Record<string, string | number>;

const en: MessageDict = {
  "common.settings": "Settings",
  "common.signIn": "Sign in",
  "common.createAccount": "Create account",
  "common.register": "Register",
  "common.goToApp": "Go to app",
  "common.installApp": "Install app",
  "common.themeTitle": "Theme: {theme}",
  "common.theme.system": "System",
  "common.theme.light": "Light",
  "common.theme.dark": "Dark",
  "common.loading": "Loading...",
  "common.working": "Working...",
  "common.refresh": "Refresh",
  "common.connect": "Connect",
  "common.disconnect": "Disconnect",
  "common.cancel": "Cancel",
  "common.stopReconnect": "Stop reconnect",
  "common.save": "Save",
  "common.reload": "Reload",
  "common.clear": "Clear",
  "common.open": "Open",
  "common.stop": "Stop",
  "common.send": "Send",
  "common.project": "Project",
  "common.model": "Model",
  "common.code": "Code",
  "common.plan": "Plan",
  "common.img": "Img",
  "common.newSession": "New session",
  "common.language": "Language",
  "common.english": "English",
  "common.chineseSimplified": "简体中文",

  "i18n.fallbackOnlyInEnglish": "English fallback only",

  "landing.title": "Codex Remote",
  "landing.heroCaptionScript": "Codex workflows",
  "landing.heroCaptionTail": "from any browser",
  "landing.heroDescription":
    "Codex Remote lets you start and supervise Codex CLI sessions running on your machine from any browser.",
  "landing.configureConnection": "Configure connection",
  "landing.configureAnchorUrl": "Configure Anchor URL",
  "landing.localModeHint": "Local mode active - no sign-in required",
  "landing.createAccount": "Create account",
  "landing.fieldNotes": "Field notes",
  "landing.featureLabel.anchor": "Anchor",
  "landing.featureLabel.orbit": "Orbit",
  "landing.featureLabel.handheld": "Handheld",
  "landing.feature.anchor":
    "A lightweight daemon that spawns and manages Codex CLI sessions. Your code stays local.",
  "landing.feature.orbit": "A Cloudflare relay that links your browser to Anchor over secure outbound tunnels.",
  "landing.feature.handheld": "Approve writes, review diffs, and control long-running tasks from a phone.",

  "auth.layout.visualWatermark": "ACCESS",
  "auth.layout.visualLabel": "Remote control for your local Codex.",
  "auth.layout.visualDesc": "Start and supervise Codex CLI sessions from any device.",

  "auth.gate.checkingSession": "Checking session...",
  "auth.gate.redirecting": "Redirecting...",
  "auth.error.backendUnavailable": "Auth backend unavailable.",
  "auth.error.signInFailed": "Sign-in failed.",
  "auth.error.unableToRequestSignIn": "Unable to request sign-in.",
  "auth.error.signInCancelled": "Sign-in cancelled.",
  "auth.error.registrationFailed": "Registration failed.",
  "auth.error.unableToStartRegistration": "Unable to start registration.",
  "auth.error.registrationCancelled": "Registration cancelled.",
  "auth.error.unableToStartTotpSetup": "Unable to start TOTP setup.",
  "auth.error.totpSetupMissing": "TOTP setup is missing.",

  "auth.login.title": "Sign in - Codex Remote",
  "auth.login.eyebrow": "Sign in",
  "auth.login.heading": "Sign in",
  "auth.login.subtitle.basic": "Sign in with your username.",
  "auth.login.subtitle.totp": "Sign in with your username and one-time code.",
  "auth.login.subtitle.passkey": "Use your passkey to access Codex Remote.",
  "auth.login.method.passkey": "Passkey",
  "auth.login.method.totp": "TOTP",
  "auth.login.usernamePlaceholder": "Username",
  "auth.login.submit.totp": "Sign in with TOTP",
  "auth.login.submit.passkey": "Sign in with passkey",
  "auth.login.createAccount": "Create new account",

  "auth.register.title": "Create account - Codex Remote",
  "auth.register.eyebrow": "Register",
  "auth.register.heading": "Create account",
  "auth.register.subtitle.basic": "Create a username for this control-plane.",
  "auth.register.subtitle.totp": "Scan QR in your authenticator app, then confirm with a code.",
  "auth.register.subtitle.passkey": "Register a new account with a passkey.",
  "auth.register.method.passkey": "Passkey",
  "auth.register.method.totp": "TOTP",
  "auth.register.usernamePlaceholder": "Username",
  "auth.register.qrAlt": "TOTP QR code",
  "auth.register.restartSetup": "Restart setup",
  "auth.register.submit.verifyCode": "Verify code",
  "auth.register.submit.setupTotp": "Setup TOTP",
  "auth.register.submit.createPasskey": "Create passkey",
  "auth.register.backToSignIn": "Back to sign in",

  "home.title": "Codex Remote",
  "home.homepage": "Homepage",
  "home.controlCenter": "(remote) control center",
  "home.summary": "Run parallel coding windows, switch projects, and keep approvals in one place.",
  "home.inputWindows": "Input windows",
  "home.paneHint": "Each window has its own project, mode and model.",
  "home.windowLabel": "Window {id}",
  "home.starting": "Starting...",
  "home.terminalHeader": "Terminal - {id}",
  "home.terminalEmpty": "Waiting for output...",
  "home.terminalAttachTitle": "Attach images",
  "home.terminalPlaceholder": "Type message, attach image, !command, or /u command for this window",
  "home.settingsLink": "Settings",
  "home.submitError.paneNotFound": "Pane not found",
  "home.submitError.inputEmpty": "Input is empty",
  "home.submitError.selectDeviceBeforeSend": "Select a device in Settings before sending messages.",
  "home.submitError.selectedDeviceOffline": "Selected device is offline. Choose another device in Settings.",
  "home.submitError.sendFailed": "Failed to send message",
  "home.submitError.sessionNotStarted": "Session is not started for this window.",
  "home.submitError.imagesNormalOnly": "Images can be sent with normal messages only.",
  "home.submitError.ulwConfigUsage": "Usage: /u config max=30 promise=DONE",
  "home.submitError.addTaskAfterU": "Add task after /u for this window.",
  "home.submitError.bangUsage": "Usage: !<command> or !pwsh|cmd|bash|sh|zsh|fish <command>",
  "home.submitError.stopTurnFailed": "Failed to stop turn",
  "home.submitError.selectModel": "Select model",
  "home.submitError.selectProject": "Select project",
  "home.submitError.useStopInsideTerminal": "Use /u stop inside an active window terminal.",
  "home.submitError.startWindowFirstThenConfig": "Start a window session first, then run /u config.",
  "home.submitError.addTaskAfterULaunchHome": "Add task after /u when launching from Home.",
  "home.submitError.failedToCreateTask": "Failed to create task",

  "composer.placeholder": "Fix a bug, build a feature, refactor code... (or attach/paste image, !<command>, /u <task>)",
  "composer.modelLoading": "Loading models...",
  "composer.modelLoadFailed": "Failed to load models",
  "composer.modelUnavailable": "No models available",

  "recent.title": "Recent Sessions",
  "recent.loading": "Loading sessions...",
  "recent.empty": "No sessions yet. Start a task above.",
  "recent.viewAll": "View all sessions",

  "sessions.title": "Codex Remote - Sessions",
  "sessions.settingsLink": "Settings",
  "sessions.allSessions": "All sessions",
  "sessions.heading": "Sessions",
  "sessions.history": "History",
  "sessions.loading": "Loading sessions...",
  "sessions.loadingMore": "Loading more sessions...",
  "sessions.empty": "No sessions yet. Start one from Home.",
  "sessions.emptyFiltered": "No sessions match this search.",
  "sessions.archiveTitle": "Archive session",
  "sessions.searchPlaceholder": "Search sessions",
  "sessions.sortLabel": "Sort",
  "sessions.sortUpdated": "Recently updated",
  "sessions.sortNewest": "Newest first",

  "appHeader.anchorAuthorize": "Authorize device",
  "appHeader.menu": "Menu",
  "appHeader.sandbox.readOnly": "Read Only",
  "appHeader.sandbox.workspace": "Workspace",
  "appHeader.sandbox.fullAccess": "Full Access",

  "settings.title": "Settings - Codex Remote",
  "settings.kicker": "Control plane",
  "settings.heading": "SETTINGS",
  "settings.summary": "Manage connection, devices, notifications, and account-level actions.",
  "settings.section.connection": "Connection",
  "settings.section.devices": "Devices",
  "settings.section.codexConfig": "Codex Config",
  "settings.section.language": "Language",
  "settings.section.account": "Account",
  "settings.account.title.signInMethods": "Sign-in methods",
  "settings.account.hint.signInMethodsScope":
    "These methods only affect web sign-in for this account. Device authorization from /device stays unchanged.",
  "settings.account.passkey.bound": "Bound",
  "settings.account.passkey.unbound": "Not bound",
  "settings.account.passkey.bind": "Bind passkey",
  "settings.account.totp.bound": "Bound",
  "settings.account.totp.unbound": "Not bound",
  "settings.account.totp.bind": "Bind TOTP",
  "settings.account.totp.rebind": "Rebind TOTP",
  "settings.account.totp.setupHint": "Scan this QR code in your authenticator app, then confirm with a 6-digit code.",
  "settings.account.totp.codePlaceholder": "6-digit code",
  "settings.account.notice.deviceUnaffected": "Already authorized devices stay signed in.",
  "settings.account.notice.totpBound": "TOTP bound. You can keep using already authorized devices.",
  "settings.account.notice.totpRebound": "TOTP replaced. You can keep using already authorized devices.",
  "settings.account.notice.passkeyBound": "Passkey bound for this account.",
  "settings.account.confirm.totpRebind": "Replace TOTP",
  "settings.account.confirm.totpRebindDetail":
    "Your current TOTP code will stop working after you confirm the new authenticator.",
  "settings.label.anchorUrl": "Anchor URL",
  "settings.label.orbitUrl": "Orbit URL",
  "settings.hint.autoConnectPaused": "Auto-connect paused. Click Connect to resume.",
  "settings.hint.autoConnectActive": "Connection is automatic on app load. Disconnect to pause and to change the URL.",
  "settings.hint.localMode":
    "Local mode: Connect directly to Anchor on your network (e.g., via Tailscale). No Orbit authentication required.",
  "settings.hint.connectToLoadDevices": "Connect to load devices.",
  "settings.hint.noDevices":
    "No devices connected. Run codex-remote start in your terminal - a code will appear, then enter it at /device to authorise.",
  "settings.hint.selectDeviceForSessions": "Select a device. New sessions will start only on the selected device.",
  "settings.hint.deviceConnectedTitle": "Connected",
  "settings.hint.deviceMeta": "{platform} - since {since}",
  "settings.hint.selected": "Selected",
  "settings.hint.connectFirstLoadConfig": "Connect first to load config.toml.",
  "settings.hint.selectDeviceLoadConfig": "Select a device to load config.toml.",
  "settings.hint.unsavedChangesDetected": "Unsaved changes detected. Save or reload to discard.",
  "settings.hint.failedLoadConfig": "Failed to load config.toml",
  "settings.hint.connectFirstSaveConfig": "Connect first to save config.toml.",
  "settings.hint.selectDeviceSaveConfig": "Select a device to save config.toml.",
  "settings.hint.configPathEmpty": "config.toml path is empty.",
  "settings.hint.failedSaveConfig": "Failed to save config.toml",
  "settings.hint.loadedPath": "Loaded {path}",
  "settings.hint.configWillCreateAtPath": "config.toml not found. It will be created at {path} when you save.",
  "settings.hint.savedPath": "Saved {path}",
  "settings.hint.connectFirstReadEditConfig": "Connect first to read and edit config.toml.",
  "settings.hint.selectDeviceEditConfig": "Select a device to edit config.toml on that machine.",
  "settings.hint.detectedOs": "Detected OS: {os}",
  "settings.hint.editingExisting": "Editing existing file.",
  "settings.hint.fileNotExistYet": "File does not exist yet; Save will create it.",
  "settings.label.configPath": "config.toml path",
  "settings.label.configContents": "Contents",
  "settings.hint.unsavedChanges": "Unsaved changes.",
  "settings.button.signOut": "Sign out",
  "settings.language.help": "Choose UI language for page labels and controls.",

  "notifications.sectionTitle": "Notifications",
  "notifications.iosHint":
    "To receive notifications on iOS, add this app to your Home Screen: tap the share button, then Add to Home Screen.",
  "notifications.pushLabel": "Push notifications",
  "notifications.disable": "Disable",
  "notifications.test": "Test",
  "notifications.enable": "Enable",
  "notifications.unavailable":
    "Push notifications are not available{iosInstallHint}.",
  "notifications.iosInstallSuffix": " - install as a Home Screen app first",

  "release.sectionTitle": "Release Cockpit",
  "release.hint.connectFirst": "Connect first to inspect and run releases.",
  "release.hint.selectDevice": "Select a device to run release checks on that machine.",
  "release.label.repoPath": "Repo path",
  "release.label.targetRef": "Target ref",
  "release.label.tagOptional": "Tag (optional)",
  "release.label.dryRun": "Dry run",
  "release.button.inspecting": "Inspecting...",
  "release.button.inspectReadiness": "Inspect readiness",
  "release.button.starting": "Starting...",
  "release.button.startRelease": "Start release",
  "release.button.polling": "Polling...",
  "release.button.pollStatus": "Poll status",
  "release.button.stopAutoPoll": "Stop auto-poll",
  "release.button.autoPoll": "Auto-poll",
  "release.readiness": "Readiness",
  "release.ready": "Ready",
  "release.needsFixes": "Needs fixes",
  "release.branch": "Branch: {branch}",
  "release.statusLabel": "Release {releaseId}",
  "release.assets": "Assets",
  "release.links": "Links",
  "release.info.checksPassed": "Release checks passed.",
  "release.info.checksAttention": "Release checks require attention.",
  "release.info.started": "Release started ({releaseId}).",
  "release.error.inspectFailed": "Failed to inspect release readiness",
  "release.error.startFailed": "Failed to start release",
  "release.error.statusLoadFailed": "Failed to load release status",
  "release.checkStatus.pass": "Pass",
  "release.checkStatus.warn": "Warn",
  "release.checkStatus.fail": "Fail",
  "release.checkStatus.unknown": "Unknown",
  "release.lifecycle.queued": "Queued",
  "release.lifecycle.pending": "Pending",
  "release.lifecycle.starting": "Starting",
  "release.lifecycle.running": "Running",
  "release.lifecycle.completed": "Completed",
  "release.lifecycle.failed": "Failed",
  "release.lifecycle.cancelled": "Cancelled",
  "release.lifecycle.blocked": "Blocked",
  "release.lifecycle.paused": "Paused",
  "release.logLevel.debug": "Debug",
  "release.logLevel.info": "Info",
  "release.logLevel.warn": "Warn",
  "release.logLevel.error": "Error",

  "device.title": "Device authorisation - Codex Remote",
  "device.eyebrow": "Device authorisation",
  "device.loading": "Loading",
  "device.checkingSession": "Checking your session...",
  "device.signInRequired": "Sign in required",
  "device.signInRequiredSubtitle": "You need an active account session before authorising a device.",
  "device.goToSignIn": "Go to sign in",
  "device.authorised": "Device authorised",
  "device.authorisedSubtitle": "Your anchor is connected. You can close this page.",
  "device.openApp": "Open app",
  "device.connectAnchor": "Connect anchor",
  "device.enterCodeHint": "Enter the code shown in your terminal.",
  "device.authorising": "Authorising...",
  "device.authorise": "Authorise",
  "device.error.authorizationFailed": "Authorization failed.",
  "device.error.authBackendUnreachable": "Could not reach auth backend.",

  "thread.title": "Thread {id} - Codex Remote",
  "thread.settingsLink": "Settings",
  "thread.workspaceKicker": "Thread workspace",
  "thread.dialogue": "DIALOGUE",
  "thread.sessionLabel": "session {id}",
  "thread.status.live": "Live",
  "thread.status.connecting": "Connecting",
  "thread.status.reconnecting": "Reconnecting",
  "thread.status.offline": "Offline",
  "thread.meta.mode": "mode {mode}",
  "thread.meta.sandbox": "sandbox {sandbox}",
  "thread.empty.begin": "BEGIN",
  "thread.empty.noMessages": "No messages yet. Write the first prompt to start this session.",
  "thread.connection.reconnectingAuto": "Reconnecting automatically...",
  "thread.connection.retry": "Retry",
  "thread.plan.approvedProceed": "Approved. Proceed with implementation.",
  "thread.error.selectDeviceBeforeSend": "Select a device in Settings before sending messages.",
  "thread.error.selectedDeviceOffline": "Selected device is offline. Choose another device in Settings.",
  "thread.error.sendFailed": "Failed to send message",
  "thread.error.imagesNormalOnly": "Images can be sent with normal messages only.",
  "thread.error.ulwConfigUsage": "Usage: /u config max=30 promise=DONE",
  "thread.error.addTaskAfterU": "Add a task after /u, or send a normal task message first.",
  "thread.error.bangUsage": "Usage: !<command> or !pwsh|cmd|bash|sh|zsh|fish <command>",
  "thread.error.stopTurnFailed": "Failed to stop turn",
  "threads.error.failedToStartThread": "Failed to start thread",
  "threads.error.noDevicesConnected": "No devices are connected.",
  "threads.error.selectDeviceBeforeSession": "Select a device in Settings before creating a session.",
  "threads.error.selectedDeviceOffline": "Selected device is offline. Choose another device in Settings.",

  "approval.header": "Approval Required",
  "approval.action.command": "Run shell command",
  "approval.action.file": "Modify file",
  "approval.action.mcp": "Run MCP tool",
  "approval.action.other": "Perform action",
  "approval.option.proceed": "Yes, proceed",
  "approval.option.alwaysSession": "Always for session",
  "approval.option.decline": "Decline",
  "approval.option.cancelTurn": "Cancel turn",
  "approval.status.approved": "Approved",
  "approval.status.declined": "Declined",
  "approval.status.cancelled": "Cancelled",
  "approval.description.fileChangeRequired": "File change requires approval",
  "approval.description.commandExecutionRequired": "Command execution requires approval",
  "approval.description.mcpToolCallRequired": "MCP tool call requires approval",
  "approval.description.actionRequired": "Action requires approval",

  "userInput.header.questions": "Questions",
  "userInput.placeholder.answer": "Type your answer...",
  "userInput.submit": "Submit",
  "userInput.answered": "Answered",

  "prompt.composerKicker": "Thread Composer",
  "prompt.composerWhisper": "paste image • !command • /u task",
  "prompt.placeholder": "Describe the next step for the agent.",
  "prompt.title.startUlw": "Start UltraWork loop",
  "prompt.title.stopUlw": "Stop ULW loop",
  "prompt.title.attachImages": "Attach images",
  "prompt.loading": "Loading...",
  "prompt.noModels": "No models available",
  "prompt.stop": "Stop",
  "prompt.modelFallback": "Model",
  "prompt.reasoning.low": "Low",
  "prompt.reasoning.medium": "Medium",
  "prompt.reasoning.high": "High",
  "prompt.mode.code": "Code",
  "prompt.mode.plan": "Plan",

  "git.panelKicker": "Thread data",
  "git.title": "Git Changes",
  "git.threadLabel": "Thread {id}",
  "git.refresh": "Refresh",
  "git.refreshing": "Refreshing...",
  "git.pathPlaceholder": "D:\\project\\repo (or absolute file path)",
  "git.status.clean": "clean",
  "git.status.dirty": "dirty",
  "git.status.branch": "branch {branch}",
  "git.status.detached": "detached",
  "git.status.changes": "changes {count}",
  "git.status.selected": "selected {count}",
  "git.tab.changes": "Changes",
  "git.tab.history": "History",
  "git.selectAll": "Select all",
  "git.revertSelected": "Revert selected",
  "git.revertAll": "Revert all",
  "git.diff": "Diff",
  "git.revert": "Revert",
  "git.selectFileForDiff": "select file for diff",
  "git.binary": "binary",
  "git.truncated": "truncated",
  "git.loadingDiff": "Loading diff...",
  "git.pickFilePreview": "Pick a file and press Diff to preview changes.",
  "git.noDiffPayloadPath": "No diff payload for this path.",
  "git.commitMessage": "Commit message",
  "git.pushAfterCommit": "Push after commit",
  "git.committing": "Committing...",
  "git.commitSelected": "Commit Selected",
  "git.pushing": "Pushing...",
  "git.push": "Push",
  "git.noLocalChanges": "No local changes in this repository.",
  "git.reloadGraph": "Reload graph",
  "git.loadingGraph": "Loading git graph...",
  "git.noCommits": "No commits yet.",
  "git.graphTruncated": "Graph output truncated to latest commits.",
  "git.error.setPathFirst": "Set absolute file or repository path first.",
  "git.error.refreshFirstResolveRoot": "Refresh status first to resolve repository root.",
  "git.error.commitMessageRequired": "Commit message is required.",
  "git.error.selectAtLeastOneCommit": "Select at least one file to commit.",
  "git.error.selectAtLeastOneRevert": "Select at least one file to revert.",
  "git.error.readStatusFailed": "Failed to read git status.",
  "git.error.loadDiffFailed": "Failed to load file diff.",
  "git.error.loadGraphFailed": "Failed to load git graph.",
  "git.error.commitFailed": "Commit failed.",
  "git.error.pushFailed": "Push failed.",
  "git.error.revertFailed": "Revert failed.",
  "git.error.revertAllFailed": "Revert all failed.",
  "git.action.committed": "Committed.",
  "git.action.pushed": "Pushed.",
  "git.action.revertedCount": "Reverted {count} file(s).",
  "git.action.revertedPath": "Reverted {path}.",
  "git.action.revertedAll": "All changes reverted.",
  "git.confirm.revertSelected": "Revert selected changes in {count} file(s)?",
  "git.confirm.revertPath": "Revert changes in {path}?",
  "git.confirm.revertAll": "Revert all local changes and remove all untracked files?",

  "artifacts.panelKicker": "Thread data",
  "artifacts.title": "Artifacts Timeline",
  "artifacts.threadLabel": "Thread {id}",
  "artifacts.refresh": "Refresh",
  "artifacts.refreshing": "Refreshing...",
  "artifacts.loading": "Loading artifacts timeline...",
  "artifacts.empty": "No artifacts yet for this thread.",
  "artifacts.error.loadFailed": "Failed to load artifacts",
  "artifacts.type.artifact": "Artifact",
  "artifacts.type.command": "Command",
  "artifacts.type.fileChange": "File Change",
  "artifacts.type.image": "Image",
  "artifacts.type.toolCall": "Tool Call",
  "artifacts.type.webSearch": "Web Search",
  "artifacts.type.collabCall": "Collab Call",
  "artifacts.status.queued": "Queued",
  "artifacts.status.pending": "Pending",
  "artifacts.status.starting": "Starting",
  "artifacts.status.running": "Running",
  "artifacts.status.completed": "Completed",
  "artifacts.status.ready": "Ready",
  "artifacts.status.failed": "Failed",
  "artifacts.status.cancelled": "Cancelled",
  "artifacts.status.blocked": "Blocked",
  "artifacts.status.paused": "Paused",

  "projectPicker.browseDirectories": "Browse directories",
  "projectPicker.closeBrowser": "Close browser",
  "projectPicker.select": "Select",
  "projectPicker.loading": "Loading...",
  "projectPicker.noSubdirectories": "No subdirectories",
  "projectPicker.error.listDirsFailed": "Failed to list directories",

  "worktreeModal.close": "Close",
  "worktreeModal.project": "Project",
  "worktreeModal.worktree": "Worktree",
  "worktreeModal.searchStartDirectory": "Search start directory",
  "worktreeModal.open": "Open",
  "worktreeModal.useCurrent": "Use current",
  "worktreeModal.inspecting": "inspecting...",
  "worktreeModal.loading": "Loading...",
  "worktreeModal.noSubdirectories": "No subdirectories",
  "worktreeModal.gitRepositoryDetected": "Git repository detected",
  "worktreeModal.newWorktreeRootOptional": "New worktree root (optional)",
  "worktreeModal.exactNewWorktreePathOptional": "Exact new worktree path (optional)",
  "worktreeModal.placeholder.searchStart": "C:\\Projects or /Users/name/projects",
  "worktreeModal.placeholder.rootDir": "D:\\codex-worktrees or /Volumes/worktrees",
  "worktreeModal.placeholder.exactPath": "D:\\codex-worktrees\\repo\\branch-name",
  "worktreeModal.cancel": "Cancel",
  "worktreeModal.next": "Next",
  "worktreeModal.back": "Back",
  "worktreeModal.select": "Select",
  "worktreeModal.newFromHead": "+ New from HEAD",
  "worktreeModal.detached": "detached",
  "worktreeModal.action.createdWorktree": "Created worktree",
  "worktreeModal.error.listDirsFailed": "Failed to list directories",
  "worktreeModal.error.createWorktreeFailed": "Failed to create worktree",
  "worktrees.error.inspectFailed": "Failed to inspect git repository",
  "worktrees.error.refreshFailed": "Failed to refresh worktrees",
  "worktrees.error.createFailed": "Failed to create worktree",
  "worktrees.error.removeFailed": "Failed to remove worktree",
  "worktrees.error.pruneFailed": "Failed to prune worktrees",

  "planCard.header": "Proposed Plan",
  "planCard.status.plan": "Plan",
  "planCard.approve": "Approve",
  "planCard.footerHint": "or reply with changes",
  "planCard.status.approved": "Approved",

  "working.label": "Working",
  "reasoning.thinking": "Thinking...",
  "reasoning.thoughtForFewSeconds": "Thought for a few seconds",

  "message.contextCompacted": "Context compacted",
  "message.reviewStarted": "Review started.",
  "message.reviewStartedWithName": "Review started: {review}",
  "message.reviewComplete": "Review complete.",
  "message.statusLabel": "Status: {status}",

  "socket.error.noServerUrlConfigured": "No server URL configured. Set one in Settings.",
  "socket.error.invalidUrl": "Invalid URL: {url}",
  "socket.error.connectionLost": "Connection lost",
  "socket.error.failedToConnect": "Failed to connect",
  "socket.error.rpc": "RPC error",
  "socket.send.notConnected": "Not connected",
  "socket.send.failed": "Failed to send",
  "socket.rpc.timeout": "RPC timeout",
  "socket.rpc.connectionClosed": "Connection closed",

  "tool.title.command": "Command",
  "tool.title.file": "File Change",
  "tool.title.fileChanges": "File changes ({count})",
  "tool.title.fileFallback": "File",
  "tool.title.mcp": "MCP Tool",
  "tool.title.web": "Web Search",
  "tool.title.image": "Image",
  "tool.title.review": "Review",
  "tool.title.plan": "Proposed Plan",
  "tool.title.agent": "Agent",
  "tool.title.default": "Tool",
  "tool.status.done": "Done",
  "tool.status.exit": "Exit {code}",
  "tool.image.loading": "Loading image...",
  "tool.image.unavailable": "Image unavailable.",
  "tool.file.summary": "Files {count}",
  "tool.file.noDetails": "No file changes details available.",
  "tool.file.open": "Open file",
  "tool.file.tab.diff": "Diff",
  "tool.file.tab.file": "File",
  "tool.file.noDiffPayload": "No diff payload for this file.",
  "tool.file.loading": "Loading file...",
  "tool.file.tooLarge": "File is too large and was not loaded.",
  "tool.error.readFileFailed": "Failed to read file.",
  "tool.error.noImageSource": "No image source available.",
  "tool.error.loadImageFailed": "Failed to load image.",
};

const zhCN: MessageDict = {
  "common.settings": "设置",
  "common.signIn": "登录",
  "common.createAccount": "创建账户",
  "common.register": "注册",
  "common.goToApp": "进入应用",
  "common.installApp": "安装应用",
  "common.themeTitle": "主题：{theme}",
  "common.theme.system": "跟随系统",
  "common.theme.light": "浅色",
  "common.theme.dark": "深色",
  "common.loading": "加载中...",
  "common.working": "处理中...",
  "common.refresh": "刷新",
  "common.connect": "连接",
  "common.disconnect": "断开连接",
  "common.cancel": "取消",
  "common.stopReconnect": "停止重连",
  "common.save": "保存",
  "common.reload": "重新加载",
  "common.clear": "清空",
  "common.open": "打开",
  "common.stop": "停止",
  "common.send": "发送",
  "common.project": "项目",
  "common.model": "模型",
  "common.code": "代码",
  "common.plan": "规划",
  "common.img": "图片",
  "common.newSession": "新会话",
  "common.language": "语言",
  "common.english": "English",
  "common.chineseSimplified": "简体中文",

  "landing.title": "Codex Remote",
  "landing.heroCaptionScript": "Codex 工作流",
  "landing.heroCaptionTail": "随时随地浏览器可达",
  "landing.heroDescription": "Codex Remote 让你从任意浏览器启动并监管本机运行的 Codex CLI 会话。",
  "landing.configureConnection": "配置连接",
  "landing.configureAnchorUrl": "配置 Anchor 地址",
  "landing.localModeHint": "本地模式已启用，无需登录",
  "landing.createAccount": "创建账户",
  "landing.fieldNotes": "功能速览",
  "landing.featureLabel.anchor": "Anchor",
  "landing.featureLabel.orbit": "Orbit",
  "landing.featureLabel.handheld": "手持端",
  "landing.feature.anchor": "Anchor 是轻量守护进程，用于启动和管理 Codex CLI 会话，代码始终保留在本地。",
  "landing.feature.orbit": "Orbit 是 Cloudflare 中继层，通过安全出站隧道把浏览器与 Anchor 连接起来。",
  "landing.feature.handheld": "你可以在手机上审批写入、查看 diff，并控制长任务。",

  "auth.layout.visualWatermark": "ACCESS",
  "auth.layout.visualLabel": "你的本地 Codex 远程控制台。",
  "auth.layout.visualDesc": "在任意设备上启动并监管 Codex CLI 会话。",

  "auth.gate.checkingSession": "正在检查会话...",
  "auth.gate.redirecting": "正在跳转...",
  "auth.error.backendUnavailable": "认证服务不可用。",
  "auth.error.signInFailed": "登录失败。",
  "auth.error.unableToRequestSignIn": "无法发起登录请求。",
  "auth.error.signInCancelled": "已取消登录。",
  "auth.error.registrationFailed": "注册失败。",
  "auth.error.unableToStartRegistration": "无法开始注册流程。",
  "auth.error.registrationCancelled": "已取消注册。",
  "auth.error.unableToStartTotpSetup": "无法开始 TOTP 设置。",
  "auth.error.totpSetupMissing": "缺少 TOTP 设置信息。",

  "auth.login.title": "登录 - Codex Remote",
  "auth.login.eyebrow": "登录",
  "auth.login.heading": "登录",
  "auth.login.subtitle.basic": "使用用户名登录。",
  "auth.login.subtitle.totp": "使用用户名和一次性验证码登录。",
  "auth.login.subtitle.passkey": "使用通行密钥访问 Codex Remote。",
  "auth.login.method.passkey": "通行密钥",
  "auth.login.method.totp": "TOTP",
  "auth.login.usernamePlaceholder": "用户名",
  "auth.login.submit.totp": "使用 TOTP 登录",
  "auth.login.submit.passkey": "使用通行密钥登录",
  "auth.login.createAccount": "创建新账户",

  "auth.register.title": "创建账户 - Codex Remote",
  "auth.register.eyebrow": "注册",
  "auth.register.heading": "创建账户",
  "auth.register.subtitle.basic": "为该控制平面创建用户名。",
  "auth.register.subtitle.totp": "用验证器应用扫描二维码，然后输入验证码确认。",
  "auth.register.subtitle.passkey": "使用通行密钥注册新账户。",
  "auth.register.method.passkey": "通行密钥",
  "auth.register.method.totp": "TOTP",
  "auth.register.usernamePlaceholder": "用户名",
  "auth.register.qrAlt": "TOTP 二维码",
  "auth.register.restartSetup": "重新开始设置",
  "auth.register.submit.verifyCode": "验证验证码",
  "auth.register.submit.setupTotp": "设置 TOTP",
  "auth.register.submit.createPasskey": "创建通行密钥",
  "auth.register.backToSignIn": "返回登录",

  "home.title": "Codex Remote",
  "home.homepage": "首页",
  "home.controlCenter": "（远程）控制中心",
  "home.summary": "并行运行多个编码窗口、切换项目，并在同一处处理审批。",
  "home.inputWindows": "输入窗口",
  "home.paneHint": "每个窗口都可以独立配置项目、模式和模型。",
  "home.windowLabel": "窗口 {id}",
  "home.starting": "启动中...",
  "home.terminalHeader": "终端 - {id}",
  "home.terminalEmpty": "等待输出中...",
  "home.terminalAttachTitle": "添加图片",
  "home.terminalPlaceholder": "输入消息、附加图片、!命令，或当前窗口的 /u 命令",
  "home.settingsLink": "设置",
  "home.submitError.paneNotFound": "未找到窗口",
  "home.submitError.inputEmpty": "输入为空",
  "home.submitError.selectDeviceBeforeSend": "发送消息前请先在设置里选择设备。",
  "home.submitError.selectedDeviceOffline": "所选设备离线，请在设置中切换其他设备。",
  "home.submitError.sendFailed": "发送消息失败",
  "home.submitError.sessionNotStarted": "当前窗口尚未启动会话。",
  "home.submitError.imagesNormalOnly": "图片只能随普通消息发送。",
  "home.submitError.ulwConfigUsage": "用法：/u config max=30 promise=DONE",
  "home.submitError.addTaskAfterU": "请在 /u 后面补充任务内容。",
  "home.submitError.bangUsage": "用法：!<command> 或 !pwsh|cmd|bash|sh|zsh|fish <command>",
  "home.submitError.stopTurnFailed": "停止当前轮次失败",
  "home.submitError.selectModel": "选择模型",
  "home.submitError.selectProject": "选择项目",
  "home.submitError.useStopInsideTerminal": "请在活动窗口终端中使用 /u stop。",
  "home.submitError.startWindowFirstThenConfig": "请先启动窗口会话，再执行 /u config。",
  "home.submitError.addTaskAfterULaunchHome": "从首页发起时，请在 /u 后面补充任务。",
  "home.submitError.failedToCreateTask": "创建任务失败",

  "composer.placeholder": "修复 bug、开发功能、重构代码...（也可附加/粘贴图片、!<command>、/u <task>）",
  "composer.modelLoading": "模型加载中...",
  "composer.modelLoadFailed": "模型加载失败",
  "composer.modelUnavailable": "暂无可用模型",

  "recent.title": "最近会话",
  "recent.loading": "正在加载会话...",
  "recent.empty": "还没有会话，可先在上方创建任务。",
  "recent.viewAll": "查看全部会话",

  "sessions.title": "Codex Remote - 会话",
  "sessions.settingsLink": "设置",
  "sessions.allSessions": "全部会话",
  "sessions.heading": "会话",
  "sessions.history": "历史",
  "sessions.loading": "正在加载会话...",
  "sessions.loadingMore": "正在加载更多会话...",
  "sessions.empty": "还没有会话，可从首页发起。",
  "sessions.emptyFiltered": "没有匹配当前搜索的会话。",
  "sessions.archiveTitle": "归档会话",
  "sessions.searchPlaceholder": "搜索会话",
  "sessions.sortLabel": "排序",
  "sessions.sortUpdated": "最近更新优先",
  "sessions.sortNewest": "最新优先",

  "appHeader.anchorAuthorize": "授权设备",
  "appHeader.menu": "菜单",
  "appHeader.sandbox.readOnly": "只读",
  "appHeader.sandbox.workspace": "工作区",
  "appHeader.sandbox.fullAccess": "完全访问",

  "settings.title": "设置 - Codex Remote",
  "settings.kicker": "控制平面",
  "settings.heading": "设置",
  "settings.summary": "管理连接、设备、通知与账户级操作。",
  "settings.section.connection": "连接",
  "settings.section.devices": "设备",
  "settings.section.codexConfig": "Codex 配置",
  "settings.section.language": "语言",
  "settings.section.account": "账户",
  "settings.account.title.signInMethods": "登录方式",
  "settings.account.hint.signInMethodsScope": "这里只影响当前账户的网页登录方式，不会改变 /device 已授权的设备。",
  "settings.account.passkey.bound": "已绑定",
  "settings.account.passkey.unbound": "未绑定",
  "settings.account.passkey.bind": "绑定通行密钥",
  "settings.account.totp.bound": "已绑定",
  "settings.account.totp.unbound": "未绑定",
  "settings.account.totp.bind": "绑定 TOTP",
  "settings.account.totp.rebind": "重绑 TOTP",
  "settings.account.totp.setupHint": "请用验证器应用扫描二维码，然后输入 6 位验证码确认。",
  "settings.account.totp.codePlaceholder": "6 位验证码",
  "settings.account.notice.deviceUnaffected": "已授权设备不会被自动下线。",
  "settings.account.notice.totpBound": "TOTP 已绑定，你可以继续使用已授权设备。",
  "settings.account.notice.totpRebound": "TOTP 已更新，你可以继续使用已授权设备。",
  "settings.account.notice.passkeyBound": "已为当前账户绑定通行密钥。",
  "settings.account.confirm.totpRebind": "替换 TOTP",
  "settings.account.confirm.totpRebindDetail": "确认新验证器后，当前 TOTP 验证码会失效。",
  "settings.label.anchorUrl": "Anchor 地址",
  "settings.label.orbitUrl": "Orbit 地址",
  "settings.hint.autoConnectPaused": "自动连接已暂停，点击“连接”可恢复。",
  "settings.hint.autoConnectActive": "应用加载时会自动连接。断开连接后可暂停并修改地址。",
  "settings.hint.localMode": "本地模式：直接连接你网络内的 Anchor（例如 Tailscale），无需 Orbit 认证。",
  "settings.hint.connectToLoadDevices": "请先连接再加载设备列表。",
  "settings.hint.noDevices": "尚无已连接设备。请在终端运行 codex-remote start，看到验证码后在 /device 页面输入授权。",
  "settings.hint.selectDeviceForSessions": "请先选择设备，新会话只会在所选设备上启动。",
  "settings.hint.deviceConnectedTitle": "已连接",
  "settings.hint.deviceMeta": "{platform} - 连接于 {since}",
  "settings.hint.selected": "已选择",
  "settings.hint.connectFirstLoadConfig": "请先连接再加载 config.toml。",
  "settings.hint.selectDeviceLoadConfig": "请先选择设备再加载 config.toml。",
  "settings.hint.unsavedChangesDetected": "检测到未保存更改，请保存或重新加载后丢弃更改。",
  "settings.hint.failedLoadConfig": "加载 config.toml 失败",
  "settings.hint.connectFirstSaveConfig": "请先连接再保存 config.toml。",
  "settings.hint.selectDeviceSaveConfig": "请先选择设备再保存 config.toml。",
  "settings.hint.configPathEmpty": "config.toml 路径为空。",
  "settings.hint.failedSaveConfig": "保存 config.toml 失败",
  "settings.hint.loadedPath": "已加载 {path}",
  "settings.hint.configWillCreateAtPath": "未找到 config.toml。保存时会在 {path} 创建。",
  "settings.hint.savedPath": "已保存 {path}",
  "settings.hint.connectFirstReadEditConfig": "请先连接，再读取和编辑 config.toml。",
  "settings.hint.selectDeviceEditConfig": "请先选择设备，再编辑该机器上的 config.toml。",
  "settings.hint.detectedOs": "检测到系统：{os}",
  "settings.hint.editingExisting": "正在编辑现有文件。",
  "settings.hint.fileNotExistYet": "文件尚不存在，点击“保存”后会创建。",
  "settings.label.configPath": "config.toml 路径",
  "settings.label.configContents": "内容",
  "settings.hint.unsavedChanges": "存在未保存更改。",
  "settings.button.signOut": "退出登录",
  "settings.language.help": "选择页面标签与控件的界面语言。",

  "notifications.sectionTitle": "通知",
  "notifications.iosHint":
    "若要在 iOS 接收通知，请先把此应用添加到主屏幕：点击分享按钮，然后选择“添加到主屏幕”。",
  "notifications.pushLabel": "推送通知",
  "notifications.disable": "关闭",
  "notifications.test": "测试",
  "notifications.enable": "开启",
  "notifications.unavailable": "当前不支持推送通知{iosInstallHint}。",
  "notifications.iosInstallSuffix": "，请先安装为主屏幕应用",

  "release.sectionTitle": "发布驾驶舱",
  "release.hint.connectFirst": "请先连接再执行发布检查或发布流程。",
  "release.hint.selectDevice": "请先选择设备，再在该机器上执行发布检查。",
  "release.label.repoPath": "仓库路径",
  "release.label.targetRef": "目标分支/引用",
  "release.label.tagOptional": "标签（可选）",
  "release.label.dryRun": "演练模式",
  "release.button.inspecting": "检查中...",
  "release.button.inspectReadiness": "检查就绪状态",
  "release.button.starting": "启动中...",
  "release.button.startRelease": "开始发布",
  "release.button.polling": "轮询中...",
  "release.button.pollStatus": "查询状态",
  "release.button.stopAutoPoll": "停止自动轮询",
  "release.button.autoPoll": "自动轮询",
  "release.readiness": "就绪度",
  "release.ready": "已就绪",
  "release.needsFixes": "需要修复",
  "release.branch": "分支：{branch}",
  "release.statusLabel": "发布 {releaseId}",
  "release.assets": "产物",
  "release.links": "链接",
  "release.info.checksPassed": "发布检查已通过。",
  "release.info.checksAttention": "发布检查需要关注。",
  "release.info.started": "发布已启动（{releaseId}）。",
  "release.error.inspectFailed": "检查发布就绪状态失败",
  "release.error.startFailed": "启动发布失败",
  "release.error.statusLoadFailed": "加载发布状态失败",
  "release.checkStatus.pass": "通过",
  "release.checkStatus.warn": "警告",
  "release.checkStatus.fail": "失败",
  "release.checkStatus.unknown": "未知",
  "release.lifecycle.queued": "已排队",
  "release.lifecycle.pending": "等待中",
  "release.lifecycle.starting": "启动中",
  "release.lifecycle.running": "进行中",
  "release.lifecycle.completed": "已完成",
  "release.lifecycle.failed": "失败",
  "release.lifecycle.cancelled": "已取消",
  "release.lifecycle.blocked": "已阻塞",
  "release.lifecycle.paused": "已暂停",
  "release.logLevel.debug": "调试",
  "release.logLevel.info": "信息",
  "release.logLevel.warn": "警告",
  "release.logLevel.error": "错误",

  "device.title": "设备授权 - Codex Remote",
  "device.eyebrow": "设备授权",
  "device.loading": "加载中",
  "device.checkingSession": "正在检查你的会话...",
  "device.signInRequired": "需要登录",
  "device.signInRequiredSubtitle": "授权设备前需要先拥有有效的账户会话。",
  "device.goToSignIn": "前往登录",
  "device.authorised": "设备已授权",
  "device.authorisedSubtitle": "你的 Anchor 已连接成功，现在可以关闭此页面。",
  "device.openApp": "打开应用",
  "device.connectAnchor": "连接 Anchor",
  "device.enterCodeHint": "请输入终端中显示的授权码。",
  "device.authorising": "授权中...",
  "device.authorise": "授权",
  "device.error.authorizationFailed": "授权失败。",
  "device.error.authBackendUnreachable": "无法连接认证后端。",

  "thread.title": "线程 {id} - Codex Remote",
  "thread.settingsLink": "设置",
  "thread.workspaceKicker": "线程工作区",
  "thread.dialogue": "对话",
  "thread.sessionLabel": "会话 {id}",
  "thread.status.live": "在线",
  "thread.status.connecting": "连接中",
  "thread.status.reconnecting": "重连中",
  "thread.status.offline": "离线",
  "thread.meta.mode": "模式 {mode}",
  "thread.meta.sandbox": "沙箱 {sandbox}",
  "thread.empty.begin": "开始",
  "thread.empty.noMessages": "还没有消息，输入第一条提示以启动本次会话。",
  "thread.connection.reconnectingAuto": "正在自动重连...",
  "thread.connection.retry": "重试",
  "thread.plan.approvedProceed": "已批准，请继续实施。",
  "thread.error.selectDeviceBeforeSend": "发送消息前请先在设置中选择设备。",
  "thread.error.selectedDeviceOffline": "所选设备离线，请在设置中选择其他设备。",
  "thread.error.sendFailed": "发送消息失败",
  "thread.error.imagesNormalOnly": "图片只能随普通消息发送。",
  "thread.error.ulwConfigUsage": "用法：/u config max=30 promise=DONE",
  "thread.error.addTaskAfterU": "请在 /u 后补充任务，或先发送普通任务消息。",
  "thread.error.bangUsage": "用法：!<command> 或 !pwsh|cmd|bash|sh|zsh|fish <command>",
  "thread.error.stopTurnFailed": "停止当前轮次失败",
  "threads.error.failedToStartThread": "启动线程失败",
  "threads.error.noDevicesConnected": "当前没有已连接设备。",
  "threads.error.selectDeviceBeforeSession": "创建会话前请先在设置中选择设备。",
  "threads.error.selectedDeviceOffline": "所选设备离线，请在设置中选择其他设备。",

  "approval.header": "需要审批",
  "approval.action.command": "运行 Shell 命令",
  "approval.action.file": "修改文件",
  "approval.action.mcp": "运行 MCP 工具",
  "approval.action.other": "执行操作",
  "approval.option.proceed": "是，继续",
  "approval.option.alwaysSession": "本会话内始终允许",
  "approval.option.decline": "拒绝",
  "approval.option.cancelTurn": "取消当前轮次",
  "approval.status.approved": "已批准",
  "approval.status.declined": "已拒绝",
  "approval.status.cancelled": "已取消",
  "approval.description.fileChangeRequired": "文件变更需要审批",
  "approval.description.commandExecutionRequired": "命令执行需要审批",
  "approval.description.mcpToolCallRequired": "MCP 工具调用需要审批",
  "approval.description.actionRequired": "该操作需要审批",

  "userInput.header.questions": "问题",
  "userInput.placeholder.answer": "请输入你的回答...",
  "userInput.submit": "提交",
  "userInput.answered": "已回答",

  "prompt.composerKicker": "线程输入区",
  "prompt.composerWhisper": "粘贴图片 • !命令 • /u 任务",
  "prompt.placeholder": "描述给 agent 的下一步。",
  "prompt.title.startUlw": "启动 UltraWork 循环",
  "prompt.title.stopUlw": "停止 ULW 循环",
  "prompt.title.attachImages": "添加图片",
  "prompt.loading": "加载中...",
  "prompt.noModels": "暂无可用模型",
  "prompt.stop": "停止",
  "prompt.modelFallback": "模型",
  "prompt.reasoning.low": "低",
  "prompt.reasoning.medium": "中",
  "prompt.reasoning.high": "高",
  "prompt.mode.code": "代码",
  "prompt.mode.plan": "规划",

  "git.panelKicker": "线程数据",
  "git.title": "Git 变更",
  "git.threadLabel": "线程 {id}",
  "git.refresh": "刷新",
  "git.refreshing": "刷新中...",
  "git.pathPlaceholder": "D:\\project\\repo（或绝对文件路径）",
  "git.status.clean": "干净",
  "git.status.dirty": "有改动",
  "git.status.branch": "分支 {branch}",
  "git.status.detached": "游离头指针",
  "git.status.changes": "改动 {count}",
  "git.status.selected": "已选 {count}",
  "git.tab.changes": "改动",
  "git.tab.history": "历史",
  "git.selectAll": "全选",
  "git.revertSelected": "回退所选",
  "git.revertAll": "全部回退",
  "git.diff": "差异",
  "git.revert": "回退",
  "git.selectFileForDiff": "选择文件查看差异",
  "git.binary": "二进制",
  "git.truncated": "已截断",
  "git.loadingDiff": "正在加载差异...",
  "git.pickFilePreview": "选择文件并点击“差异”以预览改动。",
  "git.noDiffPayloadPath": "该路径没有差异内容。",
  "git.commitMessage": "提交信息",
  "git.pushAfterCommit": "提交后推送",
  "git.committing": "提交中...",
  "git.commitSelected": "提交所选",
  "git.pushing": "推送中...",
  "git.push": "推送",
  "git.noLocalChanges": "该仓库当前没有本地改动。",
  "git.reloadGraph": "重新加载图谱",
  "git.loadingGraph": "正在加载 Git 图谱...",
  "git.noCommits": "还没有提交记录。",
  "git.graphTruncated": "图谱输出已截断为最新提交。",
  "git.error.setPathFirst": "请先填写绝对文件路径或仓库路径。",
  "git.error.refreshFirstResolveRoot": "请先刷新状态以解析仓库根目录。",
  "git.error.commitMessageRequired": "提交信息不能为空。",
  "git.error.selectAtLeastOneCommit": "请至少选择一个文件进行提交。",
  "git.error.selectAtLeastOneRevert": "请至少选择一个文件进行回退。",
  "git.error.readStatusFailed": "读取 Git 状态失败。",
  "git.error.loadDiffFailed": "加载文件差异失败。",
  "git.error.loadGraphFailed": "加载 Git 图谱失败。",
  "git.error.commitFailed": "提交失败。",
  "git.error.pushFailed": "推送失败。",
  "git.error.revertFailed": "回退失败。",
  "git.error.revertAllFailed": "全部回退失败。",
  "git.action.committed": "已提交。",
  "git.action.pushed": "已推送。",
  "git.action.revertedCount": "已回退 {count} 个文件。",
  "git.action.revertedPath": "已回退 {path}。",
  "git.action.revertedAll": "已回退全部改动。",
  "git.confirm.revertSelected": "确认回退已选的 {count} 个文件改动吗？",
  "git.confirm.revertPath": "确认回退 {path} 的改动吗？",
  "git.confirm.revertAll": "确认回退所有本地改动并删除所有未跟踪文件吗？",

  "artifacts.panelKicker": "线程数据",
  "artifacts.title": "产物时间线",
  "artifacts.threadLabel": "线程 {id}",
  "artifacts.refresh": "刷新",
  "artifacts.refreshing": "刷新中...",
  "artifacts.loading": "正在加载产物时间线...",
  "artifacts.empty": "该线程暂时没有产物。",
  "artifacts.error.loadFailed": "加载产物失败",
  "artifacts.type.artifact": "产物",
  "artifacts.type.command": "命令",
  "artifacts.type.fileChange": "文件变更",
  "artifacts.type.image": "图片",
  "artifacts.type.toolCall": "工具调用",
  "artifacts.type.webSearch": "网页搜索",
  "artifacts.type.collabCall": "协作调用",
  "artifacts.status.queued": "已排队",
  "artifacts.status.pending": "等待中",
  "artifacts.status.starting": "启动中",
  "artifacts.status.running": "进行中",
  "artifacts.status.completed": "已完成",
  "artifacts.status.ready": "已就绪",
  "artifacts.status.failed": "失败",
  "artifacts.status.cancelled": "已取消",
  "artifacts.status.blocked": "已阻塞",
  "artifacts.status.paused": "已暂停",

  "projectPicker.browseDirectories": "浏览目录",
  "projectPicker.closeBrowser": "关闭浏览器",
  "projectPicker.select": "选择",
  "projectPicker.loading": "加载中...",
  "projectPicker.noSubdirectories": "没有子目录",
  "projectPicker.error.listDirsFailed": "读取目录列表失败",

  "worktreeModal.close": "关闭",
  "worktreeModal.project": "项目",
  "worktreeModal.worktree": "工作树",
  "worktreeModal.searchStartDirectory": "搜索起始目录",
  "worktreeModal.open": "打开",
  "worktreeModal.useCurrent": "使用当前目录",
  "worktreeModal.inspecting": "检查中...",
  "worktreeModal.loading": "加载中...",
  "worktreeModal.noSubdirectories": "没有子目录",
  "worktreeModal.gitRepositoryDetected": "检测到 Git 仓库",
  "worktreeModal.newWorktreeRootOptional": "新工作树根目录（可选）",
  "worktreeModal.exactNewWorktreePathOptional": "新工作树精确路径（可选）",
  "worktreeModal.placeholder.searchStart": "例如：C:\\Projects 或 /Users/name/projects",
  "worktreeModal.placeholder.rootDir": "例如：D:\\codex-worktrees 或 /Volumes/worktrees",
  "worktreeModal.placeholder.exactPath": "例如：D:\\codex-worktrees\\repo\\branch-name",
  "worktreeModal.cancel": "取消",
  "worktreeModal.next": "下一步",
  "worktreeModal.back": "返回",
  "worktreeModal.select": "选择",
  "worktreeModal.newFromHead": "+ 从 HEAD 新建",
  "worktreeModal.detached": "游离头指针",
  "worktreeModal.action.createdWorktree": "已创建工作树",
  "worktreeModal.error.listDirsFailed": "读取目录列表失败",
  "worktreeModal.error.createWorktreeFailed": "创建工作树失败",
  "worktrees.error.inspectFailed": "检查 Git 仓库失败",
  "worktrees.error.refreshFailed": "刷新工作树失败",
  "worktrees.error.createFailed": "创建工作树失败",
  "worktrees.error.removeFailed": "移除工作树失败",
  "worktrees.error.pruneFailed": "清理工作树失败",

  "planCard.header": "建议计划",
  "planCard.status.plan": "计划",
  "planCard.approve": "批准",
  "planCard.footerHint": "或直接回复修改建议",
  "planCard.status.approved": "已批准",

  "working.label": "处理中",
  "reasoning.thinking": "思考中...",
  "reasoning.thoughtForFewSeconds": "思考了几秒钟",

  "message.contextCompacted": "上下文已压缩",
  "message.reviewStarted": "审查已开始。",
  "message.reviewStartedWithName": "审查已开始：{review}",
  "message.reviewComplete": "审查已完成。",
  "message.statusLabel": "状态：{status}",

  "socket.error.noServerUrlConfigured": "未配置服务端 URL，请在设置页填写。",
  "socket.error.invalidUrl": "无效 URL：{url}",
  "socket.error.connectionLost": "连接已断开",
  "socket.error.failedToConnect": "连接失败",
  "socket.error.rpc": "RPC 错误",
  "socket.send.notConnected": "未连接",
  "socket.send.failed": "发送失败",
  "socket.rpc.timeout": "RPC 超时",
  "socket.rpc.connectionClosed": "连接已关闭",

  "tool.title.command": "命令",
  "tool.title.file": "文件变更",
  "tool.title.fileChanges": "文件变更（{count}）",
  "tool.title.fileFallback": "文件",
  "tool.title.mcp": "MCP 工具",
  "tool.title.web": "网页搜索",
  "tool.title.image": "图片",
  "tool.title.review": "审查",
  "tool.title.plan": "建议计划",
  "tool.title.agent": "智能体",
  "tool.title.default": "工具",
  "tool.status.done": "完成",
  "tool.status.exit": "退出码 {code}",
  "tool.image.loading": "正在加载图片...",
  "tool.image.unavailable": "图片不可用。",
  "tool.file.summary": "文件 {count}",
  "tool.file.noDetails": "暂无文件变更明细。",
  "tool.file.open": "打开文件",
  "tool.file.tab.diff": "差异",
  "tool.file.tab.file": "文件",
  "tool.file.noDiffPayload": "该文件没有差异内容。",
  "tool.file.loading": "正在加载文件...",
  "tool.file.tooLarge": "文件过大，未加载内容。",
  "tool.error.readFileFailed": "读取文件失败。",
  "tool.error.noImageSource": "没有可用图片来源。",
  "tool.error.loadImageFailed": "加载图片失败。",
};

const MESSAGES: Record<Locale, MessageDict> = {
  en,
  "zh-CN": zhCN,
};

function isLocale(value: string | null): value is Locale {
  return value !== null && SUPPORTED_LOCALES.includes(value as Locale);
}

function applyDocumentLocale(locale: Locale) {
  if (typeof document === "undefined") return;
  document.documentElement.lang = locale;
}

function interpolate(template: string, params?: TranslationParams): string {
  if (!params) return template;
  return template.replace(/\{(\w+)\}/g, (match, paramName: string) => {
    if (!(paramName in params)) return match;
    return String(params[paramName]);
  });
}

class I18nStore {
  #locale = $state<Locale>("en");

  constructor() {
    if (typeof localStorage !== "undefined") {
      const savedLocale = localStorage.getItem(STORAGE_KEY);
      if (isLocale(savedLocale)) {
        this.#locale = savedLocale;
      }
    }
    applyDocumentLocale(this.#locale);
  }

  get current(): Locale {
    return this.#locale;
  }

  get supportedLocales(): Locale[] {
    return [...SUPPORTED_LOCALES];
  }

  set(locale: Locale) {
    if (locale === this.#locale) return;
    this.#locale = locale;
    if (typeof localStorage !== "undefined") {
      localStorage.setItem(STORAGE_KEY, locale);
    }
    applyDocumentLocale(locale);
  }

  t(key: string, params?: TranslationParams): string {
    const localeMessages = MESSAGES[this.#locale];
    const template = localeMessages[key] ?? MESSAGES.en[key] ?? key;
    return interpolate(template, params);
  }

  themeName(theme: string): string {
    if (theme === "system") return this.t("common.theme.system");
    if (theme === "light") return this.t("common.theme.light");
    if (theme === "dark") return this.t("common.theme.dark");
    return theme;
  }

  formatDate(value: Date | number | string, options?: Intl.DateTimeFormatOptions): string {
    const date = value instanceof Date ? value : new Date(value);
    if (!Number.isFinite(date.getTime())) return "";
    return new Intl.DateTimeFormat(this.#locale, options).format(date);
  }
}

function getStore(): I18nStore {
  const global = globalThis as Record<string, unknown>;
  if (!global[STORE_KEY]) {
    global[STORE_KEY] = new I18nStore();
  }
  return global[STORE_KEY] as I18nStore;
}

export const i18n = getStore();
