...
- 2026-03-26T11:02:49+08:00 [agent] worktrees 相关 UI 统一复用 store 导出的错误归一化/渲染 helper（`toWorktreesUiMessage` + `renderWorktreesUiMessage`），避免组件层重复写 `err.message || i18n.t(...)` 导致提示固化。
